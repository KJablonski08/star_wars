<template>
	<div>
    <!-- SELECT FORM --> 
        <h4 class="mt-3">Welcome, Select the Star Wars data would you like to know more about...</h4>
        <b-form-select v-model="selected" :options="options" size="sm" class="mt-3 w-50 p-3 mb-1 bg-secondary text-light" v-on:change="updateResults">
        </b-form-select>
        <div class="mt-3 mb-3" v-if="selected">Selected: <strong>{{ selected.toUpperCase() }}</strong></div>
        <div class="d-flex justify-content-center">
            <b-form-input list="my-list-id"  v-show="results && selected !== 'films'" class="w-50 mb-3" v-on:input="filterList" placeholder="Filter here..."></b-form-input>
